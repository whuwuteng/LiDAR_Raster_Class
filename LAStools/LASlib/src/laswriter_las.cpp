...
BOOL LASwriterLAS::open(ByteStreamOut* stream, const LASheader* header, U32 compressor, I32 requested_version, I32 chunk_size)
{
  ...
  // add 2020/10/28 by wuteng
  pheader = (LASheader *)header;
  if (stream == 0)
  {
...
// add 2020/10/28 by wuteng
void LASwriterLAS::Offset( int nIndex )
{
	int nOffset = pheader->offset_to_point_data + nIndex * pheader->point_data_record_length;
